...
void Testbed::set_mask_weight(float weight) {
    m_mask_loss_weight = weight;
}


void Testbed::apply_normalization() {
    m_apply_normalization = true;
}

void Testbed::apply_light_opti() {
    m_light_opti = true;
}
...
void Testbed::apply_rgbplus(){
	m_apply_rgbplus = true;
}
		...
		if (ImGui::CollapsingHeader("Marching Cubes Mesh Output")) {
			...
			if (uint32_t tricount = m_mesh.indices.size()/3) {
				...
				if (ImGui::Button("Save it!")) {
					save_mesh(m_mesh.verts, m_mesh.vert_normals, m_mesh.vert_colors, m_mesh.indices, obj_filename_buf, m_mesh.unwrap,
								m_nerf.training.dataset.scale, m_nerf.training.dataset.offset,
								m_nerf.training.dataset.n2w_s, m_nerf.training.dataset.n2w_t, 
								m_nerf.training.dataset.from_na);
				}
...
void Testbed::train(uint32_t batch_size) {
	...
	bool get_loss_scalar = m_training_step % 1 == 0;
