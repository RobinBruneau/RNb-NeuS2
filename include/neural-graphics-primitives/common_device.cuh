...
template <typename T>
__global__ void from_rgba64(const uint64_t num_pixels, const uint16_t* __restrict__ pixels, T* __restrict__ out, bool white_2_transparent = false, bool black_2_transparent = false, uint64_t mask_color = 0) {
	const uint64_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= num_pixels) return;
	...
	*((uint64_t*)&rgba[0]) = *((uint64_t*)&pixels[i*4]);
	...
	float alpha = rgba[3] * (1.0f/65535.0f);
	// NSVF dataset has 'white = transparent' madness
	...
	tcnn::vector_t<T, 4> rgba_out;
	rgba_out[0] = (T)(srgb_to_linear(rgba[0] * (1.0f/65535.0f)) * alpha);
	rgba_out[1] = (T)(srgb_to_linear(rgba[1] * (1.0f/65535.0f)) * alpha);
	rgba_out[2] = (T)(srgb_to_linear(rgba[2] * (1.0f/65535.0f)) * alpha);
	rgba_out[3] = (T)alpha;
	...
	switch (image_data_type) {
		...
		case EImageDataType::Byte: {
			...
			*(uint64_t*)&val[0] = ((uint64_t*)pixels)[pixel_idx(px, resolution, img)];
			...
			if (*(uint64_t*)&val[0] == 0x00FF00FF) {
				return Eigen::Array4f::Constant(-1.0f);
			}

			float alpha = (float)val[3] * (1.0f/65535.0f);
			Eigen::Array4f rgba = Eigen::Array4f{
				srgb_to_linear((float)val[0] * (1.0f/65535.0f)) * alpha,
				srgb_to_linear((float)val[1] * (1.0f/65535.0f)) * alpha,
				srgb_to_linear((float)val[2] * (1.0f/65535.0f)) * alpha,
				alpha,
			};
