...
public:
	...
	std::unique_ptr<tcnn::Context> forward_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>* output = nullptr, bool use_inference_params = false, bool prepare_input_gradients = false) override {
		...
		// copy DIR from INPUT to deformed_viewdir
		...
		// copy deformed XYZ to delta_network_output
		tcnn::linear_kernel(fill_positions_view<float,float>, 0, stream, 
		...
		// copy deformed_viewdir to delta_network_output
		tcnn::linear_kernel(fill_positions_view<float,float>, 0, stream, 
							batch_size * m_dir_encoding->input_width(), 
							m_dir_encoding->input_width(),
							deformed_viewdir.view(), 
							get_advance(forward->delta_network_output.view(), m_pos_encoding->input_width(), 0));
		...
		tcnn::linear_kernel(fill_positions_view<T, T>, 0, stream,
							...
							get_advance(dSDF_dSDFInput.view(), 
							m_pos_encoding->input_width(), 0) , dSDF_dPosEncoding.view());
		...
		tcnn::linear_kernel(fill_positions_view<T, float>, 0, stream,
							...
							m_pos_encoding->input_width(),
							deformed_xyz.view(), 
							get_advance(forward->rgb_network_input.view(), m_density_network->padded_output_width() + m_dir_encoding->padded_output_width(), 0));
		...
		tcnn::linear_kernel(fill_positions_view<T, float>, 0, stream,
							...
							get_advance(forward->rgb_network_input.view(), m_density_network->padded_output_width() + m_dir_encoding->padded_output_width() + m_pos_encoding->input_width(), 0));
	...
	void backward_impl(
		...
		//dL_ddensity_network_output.memset(0);
